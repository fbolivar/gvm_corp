...
export default function EditOrderClient({ orderId, poNumber, initialData, parties, products, warehouses }: Props) {
    ...
    function addLine() {
        append({ product_id: "", qty: 1, unit_cost: 0, tax_rate: 0, qty_received: 0, notes: "" });
    }
    ...
    async function validate(): Promise<boolean> {
        ...
        if (!valid) {
            ...
            toast.error(missing.length > 0 ? `Falta: ${missing.join(', ')}` : "Complete todos los campos requeridos");
        }
        return valid;
    }
    ...
    async function handleSave() {
        ...
        try {
            ...
            const result = await updatePurchaseOrderAction(orderId, { ...data, status: "DRAFT" });
            if (result?.error) {
    ...
    async function handleSaveAndSubmit() {
        ...
        try {
            const data = form.getValues();
            const saveResult = await updatePurchaseOrderAction(orderId, { ...data, status: "DRAFT" });
            if (saveResult?.error) {
                toast.error(saveResult.error);
                return;
            }
            const approvalResult = await submitForApprovalAction(orderId);
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <div className="w-full h-14 bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl px-12 flex items-center">
                                    <span className="text-lg font-black text-slate-900 tracking-tight font-mono">{poNumber}</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <select {...form.register("supplier_id")} className={selectClass}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <Input type="date" {...form.register("order_date")} className={inputClass} />
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <Input type="date" {...form.register("expected_delivery")} className={inputClass} />
                            </div>
                        </div>
                    </div>
                    ...
                    <CardContent className="p-10">
                        <div className="flex flex-col items-end gap-4">
                            <div className="flex items-center justify-between w-full">
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Resumen Financiero</p>
                                <span className="text-[9px] font-black text-amber-400 uppercase tracking-widest bg-amber-500/10 px-3 py-1 rounded-full">{selectedCurrency}</span>
                            </div>
                            <div className="w-full max-w-sm space-y-3">
                                <div className="flex items-center justify-between">
                                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Subtotal</span>
                                    <span className="text-sm font-bold text-slate-300 tabular-nums">{currencySymbol(selectedCurrency)} {fmt(subtotal, selectedCurrency)}</span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">IVA</span>
                                    <span className="text-sm font-bold text-amber-400 tabular-nums">{currencySymbol(selectedCurrency)} {fmt(iva, selectedCurrency)}</span>
                                </div>
                                <div className="h-px bg-white/5" />
                                <div className="flex items-center justify-between">
                                    <span className="text-[10px] font-black text-white uppercase tracking-widest">Total {selectedCurrency}</span>
                                    <span className="text-2xl font-black text-white tabular-nums tracking-tighter">{currencySymbol(selectedCurrency)} {fmt(total, selectedCurrency)}</span>
                                </div>
                            </div>
                        </div>
                    </CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Button
                    type="button"
                    disabled={loadingSave || loadingApproval}
                    onClick={handleSave}
                ...
                >
                    {loadingSave ? (
                        <><Loader2 className="h-4 w-4 mr-3 animate-spin" />Guardando...</>
                    ) : (
                ...
                >
                    {loadingApproval ? (
                        <><Loader2 className="h-4 w-4 mr-3 animate-spin" />Enviando a Aprobación...</>
                    ) : (
                        <><Send className="h-5 w-5 mr-3" />Guardar y Enviar a Aprobación</>
                    )}
                </Button>
            </div>
        </div>
...
function LineRow({ index, form, products, currency, onProductSelect, onRemove }: LineRowProps) {
    ...
    const taxRate = useWatch({ control: form.control, name: `lines.${index}.tax_rate` }) ?? 0;
    ...
    return (
        <div className="bg-slate-50 rounded-2xl p-4 md:p-0 md:bg-transparent">
            <div className="grid grid-cols-1 md:grid-cols-[2rem_1fr_8rem_10rem_8rem_10rem_3rem] gap-4 items-center">
                ...
                <div>
                    <Input
                        type="number" step="1" min="0" placeholder="Costo"
                ...
                <div className="relative">
                    <select
                        ...
                        onChange={(e) => form.setValue(`lines.${index}.tax_rate`, Number(e.target.value), { shouldValidate: true })}
                        className="w-full h-12 bg-white border-none rounded-xl px-4 text-xs font-bold text-slate-700 shadow-inner focus:ring-4 focus:ring-primary/5 outline-none cursor-pointer appearance-none"
                    >
                ...
                <div className="flex items-center justify-center">
                    <button
                        type="button" onClick={onRemove} aria-label="Eliminar línea"
