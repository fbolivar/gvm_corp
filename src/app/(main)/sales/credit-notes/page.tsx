...
import { Plus, FileX, Banknote, Activity, Link2 } from 'lucide-react';
...
export const metadata = { title: 'Notas Crédito — GVM Corp' };
...
export default async function CreditNotesPage() {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const totalValue = creditNotes.reduce((acc: number, note: any) => acc + (Number(note.total) || 0), 0);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const draftCount = creditNotes.filter((note: any) => note.status === 'DRAFT').length;

    // Fetch parent invoice numbers
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const parentIds = creditNotes.map((note: any) => note.parent_id).filter(Boolean) as string[];
    ...
    const kpis = [
        { label: 'Total Notas Crédito', value: `$${totalValue.toLocaleString('es-CO')}`, icon: Banknote, color: 'text-rose-600', bg: 'bg-rose-50' },
        { label: 'Documentos Emitidos', value: creditNotes.length, icon: FileX, color: 'text-indigo-600', bg: 'bg-indigo-50' },
        { label: 'Pendientes', value: draftCount, icon: Activity, color: 'text-amber-600', bg: 'bg-amber-50' },
    ];
    ...
    return (
        <div className="space-y-6 pb-16 animate-in fade-in duration-500">
            <VisualReportHeader
                title="Notas Crédito"
                subtitle="Ventas — Devoluciones y Descuentos Post-Facturación"
                tenant={tenant}
            />

            <div className="flex flex-wrap gap-2">
                <Button asChild className="h-9 rounded-xl bg-rose-600 hover:bg-rose-700 text-xs font-semibold gap-2">
                    <Link href="/sales/credit-notes/new">
                        <Plus className="h-3.5 w-3.5" /> Nueva NC
                    </Link>
                </Button>
                ...
                <CardContent className="p-0">
                    {creditNotes.length === 0 ? (
                        <div className="py-16 flex flex-col items-center gap-4">
                            <FileX className="h-8 w-8 text-slate-200" />
                            <div className="text-center">
                                <p className="text-sm font-semibold text-slate-900">Sin Notas Crédito</p>
