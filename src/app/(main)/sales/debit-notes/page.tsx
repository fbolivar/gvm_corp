...
import { Plus, FilePlus2, ShieldCheck, Banknote, Activity, Link2, ArrowLeft } from 'lucide-react';
...
export default async function DebitNotesPage() {
    ...
    const totalValue = debitNotes.reduce((acc: number, dn: any) => acc + (Number(dn.total) || 0), 0);
    const draftCount = debitNotes.filter((dn: any) => dn.status === 'DRAFT').length;

    const parentIds = debitNotes
        .map((dn: any) => dn.parent_id)
        ...
        <div className="page-container space-y-12 pb-24 animate-in fade-in slide-in-from-bottom-8 duration-1000">
            ...
            <div className="relative group overflow-hidden bg-slate-950 rounded-[2.5rem] p-10 text-white shadow-active">
                <div className="absolute top-0 right-0 p-12 opacity-[0.03] pointer-events-none group-hover:scale-110 group-hover:rotate-12 transition-all duration-1000">
                    <FilePlus2 className="h-24 w-24 text-white" />
                </div>
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-10">
                    <div className="space-y-4">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs font-black uppercase tracking-[0.5em] text-amber-400">
                                Ajuste de Cartera
                            </span>
                        </div>
                        <h1 className="text-3xl md:text-4xl font-black tracking-tight uppercase leading-tight">
                            Notas <br /><span className="text-slate-500">Debito</span>
                        </h1>
                        <p className="text-slate-500 font-black text-[10px] uppercase tracking-[0.4em]">
                            Cargos Adicionales Post-Facturacion (v3.0)
                        </p>
                    </div>
                    ...
                    <div className="flex gap-4 flex-wrap">
                        ...
                        <Button
                            asChild
                            className="h-12 px-12 rounded-[2rem] bg-amber-600 hover:bg-amber-500 text-white font-black shadow-active transition-all hover:scale-105 active:scale-95 border-none"
                        >
                            <Link href="/sales/debit-notes/new" className="flex items-center gap-4">
                                <Plus className="h-7 w-7" />
                                <div className="flex flex-col items-start leading-none">
                                    <span className="text-[10px] uppercase tracking-widest opacity-60">Crear</span>
                                    <span className="text-xs uppercase tracking-widest">Nueva ND</span>
                                </div>
                            </Link>
                        </Button>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {[
                    ...
                    <div
                        key={i}
                        className="bg-white p-8 rounded-[2.5rem] shadow-premium flex items-center gap-6 border border-slate-50 hover:border-amber-100 transition-colors group"
                    >
                        <div
                            className={cn(
                ...
                <div className="bg-white rounded-[3.5rem] shadow-premium overflow-hidden border border-slate-100/50">
                    {debitNotes.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-24 gap-6 text-slate-400">
                            <div className="h-20 w-20 rounded-[2rem] bg-amber-50 flex items-center justify-center">
                                <FilePlus2 className="h-10 w-10 text-amber-300" />
                            </div>
                            <div className="text-center space-y-2">
                                ...
                                <p className="text-xs text-slate-400 max-w-xs">
                                    No se han generado notas debito. Crea una desde una factura existente.
                                </p>
                            </div>
                            <Button
                                asChild
                                className="rounded-2xl bg-amber-600 hover:bg-amber-500 text-white font-black px-8 h-10"
                            >
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {debitNotes.map((dn: any, idx: number) => {
                                        const status = STATUS_LABELS[dn.status] ?? {
                                            label: dn.status,
                                            color: 'bg-slate-100 text-slate-600',
                                        };
                                        const parentNumber = dn.parent_id
                                            ? parentMap[dn.parent_id]
                                            : null;
                                            ...
                                            >
                                                ...
                                                <td className="px-6 py-5">
                                                    <span className="text-sm text-slate-600 font-bold">
                                                        {dn.issue_date
                                                            ? new Date(dn.issue_date).toLocaleDateString('es-CO')
                                                            : '—'}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-5 text-right">
                                                    <span className="font-black text-slate-900 text-sm tabular-nums">
                                                        ${Number(dn.total).toLocaleString('es-CO')}
                                                    </span>
                                                </td>
                                                ...
                                                <td className="px-8 py-5 text-center">
                                                    <Link
                                                        href={`/documents/${dn.id}`}
                                                        className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-100 hover:bg-amber-100 hover:text-amber-700 text-slate-600 font-black text-[9px] uppercase tracking-widest transition-all"
                                                    >
                                                        Ver
                                                    </Link>
                                                </td>
            ...
            <div className="bg-slate-950 p-10 rounded-[2.5rem] text-white flex flex-col lg:flex-row items-center justify-between gap-12 shadow-active relative overflow-hidden group">
                ...
                <div className="flex items-center gap-10 relative z-10 flex-col lg:flex-row text-center lg:text-left">
                    <div className="h-20 w-20 bg-white/10 rounded-[2rem] flex items-center justify-center border border-white/10 shadow-inner rotate-12 group-hover:rotate-0 transition-transform duration-700">
                        <FilePlus2 className="h-10 w-10 text-amber-400" />
                    </div>
                    <div className="space-y-3">
                        <h4 className="text-2xl font-black tracking-tight uppercase leading-tight text-white">
                            Cargos Adicionales sobre Facturas
                        </h4>
                        <p className="text-sm text-white/40 leading-relaxed font-medium max-w-xl">
                            Las notas debito incrementan el saldo exigible de la factura original. Deben referenciar
                            la factura DIAN y ser aceptadas por el cliente segun la normativa{' '}
                            <span className="text-amber-400 font-black uppercase">Resolucion 000042/2020</span>.
                        </p>
                    </div>
                </div>
            </div>
