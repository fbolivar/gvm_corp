...
import { FileText, CheckCircle2, UploadCloud, ArrowRight, Building2, Bot } from "lucide-react"
...
export function PaymentClient({ document: doc }: PaymentClientProps) {
    const [uploading, setUploading] = useState(false)
    const [selectedFile, setSelectedFile] = useState<File | null>(null)
    const [reportData, setReportData] = useState({ amount: String(doc.total ?? ""), notes: "" })
    ...
    const dueDate = doc.due_date || doc.issue_date
                    ...
                    <Card className="border-none bg-white rounded-[3rem] shadow-premium overflow-hidden">
                        <CardHeader className="p-10 border-b border-slate-100 bg-slate-50/50">
                            <div className="flex justify-between items-start">
                                <div>
                                    <Badge className="mb-4 bg-slate-900 text-white font-black italic rounded-full px-4">
                                        FACTURA #{doc.number}
                                    </Badge>
                                    <CardTitle className="text-4xl font-black text-slate-900 italic tracking-tighter uppercase">
                                        Resumen de Cuenta
                                    </CardTitle>
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-8">
                                <div className="space-y-1">
                                    ...
                                    <p className="text-3xl font-black italic text-slate-900">
                                        {doc.total.toLocaleString('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 })}
                                    </p>
                                </div>
                                <div className="space-y-1">
                                    ...
                                    <p className="text-xl font-black italic text-slate-600 uppercase">
                                        {new Date(dueDate).toLocaleDateString('es-CO', { day: 'numeric', month: 'short', year: 'numeric' })}
                                    </p>
                                </div>
                            </div>
                            ...
                            <Label className="cursor-pointer">
                                ...
                                <div className={cn(
                                    ...
                                    {selectedFile
                                        ...
                                        : <UploadCloud className="w-10 h-10 group-hover:-translate-y-2 transition-transform" />
                                    }
