...
/**
 * Botón que imprime un pedido de venta con la receta médica ICA y la firma
 * del comercial responsable. Abre popup centrado que auto-imprime y cierra.
 */
export function PrintSalesOrderButton({ docId }: { docId: string }) {
  const handlePrint = () => {
    const url = `/print/sales-order?id=${docId}`;
    const width = 900;
    const height = 1100;
