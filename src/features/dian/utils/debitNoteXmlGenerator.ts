...
interface DebitNoteXmlParams {
    document: Document;
    cufe: string; // For Debit Notes it is called CUDE but technically same format
...
<DebitNote xmlns="urn:oasis:names:specification:ubl:schema:xsd:DebitNote-2"
    ...
    <cbc:CustomizationID>92</cbc:CustomizationID>
    ...
    <cbc:DebitNoteTypeCode listAgencyID="195" listAgencyName="CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)" listID="92">92</cbc:DebitNoteTypeCode>
    <cbc:Note>Nota Débito Generada por SaaS Factory V3</cbc:Note>
    <cbc:DocumentCurrencyCode>COP</cbc:DocumentCurrencyCode>

    <cac:DiscrepancyResponse>
        <cbc:ReferenceID>${originalInvoiceNumber}</cbc:ReferenceID>
        <cbc:ResponseCode>1</cbc:ResponseCode>
        <cbc:Description>Intereses y gastos por mora</cbc:Description>
    </cac:DiscrepancyResponse>
...
const generateLineXml = (line: DocumentLine, index: number) => {
    ...
    <cac:DebitNoteLine>
        <cbc:ID>${index}</cbc:ID>
        <cbc:DebitedQuantity unitCode="94">${line.qty}</cbc:DebitedQuantity>
